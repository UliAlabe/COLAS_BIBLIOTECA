...
def simular_sistema(params):
    ...
    # [NUEVO EULER]: Separamos las listas para las dos tablas.
    datos_grilla = []  # Almacenará las tuplas finales que van a la UI Principal.
    datos_euler = []  # Almacenará las tuplas de los pasos de integración numérica.
    ...
    def proceso_cliente(id_cliente):
        ...
        if tipo == "Pedir" and se_queda == "Sí":
            ...
            # [NUEVO EULER]: Reemplazamos la integral directa por el Método de Euler.
            rnd_p = random.random()
            ...
            #Antes de empezar a integrar, el código define el escenario: paginas_target: Es la meta. El número aleatorio de páginas que tiene el libro (ej. 250).
            # h: Es el tamaño del paso de integración que definiste en la interfaz (ej. 0.1). Mientras más chico, más preciso es el cálculo de la curva, pero más
            # iteraciones de procesamiento necesita la PC. t_euler = 0.0: El cronómetro de la integración matemática arranca en cero.
            # p_actual = 0.0: El cliente arranca con 0 páginas leídas.
            ...
            #Este while es el núcleo del mét0do numérico. Le dice a Python: "Seguí iterando y sumando páginas hasta que el cliente termine el libro".
            #Adentro ocurren 4 acciones encadenadas:
            #Cálculo de la Derivada: dp_dt = k / 5.0.
            #El Salto de Euler: p_siguiente = p_actual + h * dp_dt
            #Auditoría (Pestaña 2): datos_euler.append(...)
            #Avance del Reloj y Acumulación: p_actual = p_siguiente y t_euler += h
            while p_actual < paginas_target:
                dp_dt = k / 5.0  # La derivada dada por la cátedra
                p_siguiente = p_actual + h * dp_dt

                # Guarda cada iteración del cálculo para mostrarla en la Pestaña 2
                datos_euler.append((
                    f"Cliente {id_cliente}", round(t_euler, 4), round(p_actual, 2),
                    round(dp_dt, 2), round(p_siguiente, 2), k, int(paginas_target)
                ))
    ...
    columnas_base = [
        ...
        "RND Pág", "Páginas", "Valor K", "T. Lectura",
    ...
    # [NUEVO EULER]: Definimos las columnas estrictas para la Pestaña 2 (Integración).
    columnas_euler = ("Instancia Variable (ID Cliente)", "t (U. Integración)", "Páginas Leídas (P)", "Derivada (dP/dt)",
                      "Prox. Páginas (P+1)", "K Aplicado", "Total Páginas Libro")
...
class VentanaSimulacion:
    def __init__(self, root):
        ...
        # [NUEVO EULER]: Transformamos el Frame inferior único en un sistema de Pestañas (Notebook)
        self.notebook = ttk.Notebook(self.root)
        self.notebook.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

        # Pestaña 1: Grilla Principal (Vector de Estado)
        self.tab1 = ttk.Frame(self.notebook)
        self.notebook.add(self.tab1, text="📋 Vector de Estado Principal")
        self.tree_principal = self.crear_tabla(self.tab1)

        # Pestaña 2: Grilla Integración Euler
        self.tab2 = ttk.Frame(self.notebook)
        self.notebook.add(self.tab2, text="🧮 Integración Numérica (Euler)")
        self.tree_euler = self.crear_tabla(self.tab2)
    ...
    # [NUEVO EULER]: Modularizamos la creación de tablas para no repetir el código del Scrollbar dos veces
    def crear_tabla(self, contenedor):
        scroll_y = ttk.Scrollbar(contenedor, orient=tk.VERTICAL)
        scroll_x = ttk.Scrollbar(contenedor, orient=tk.HORIZONTAL)

        tree = ttk.Treeview(contenedor, show="headings", yscrollcommand=scroll_y.set, xscrollcommand=scroll_x.set,
                            selectmode="extended")
    ...
    def ejecutar_simulacion(self):
        ...
        # [NUEVO EULER]: Recibimos los datos de ambas tablas
        col_p, datos_p, col_e, datos_e, p_rechazos, p_permanencia = simular_sistema(params)

        self.lbl_res1.config(
            text=f"Promedio de permanencia: {p_permanencia:.2f} min | Filas Vector Estado: {len(datos_p)}")
        self.lbl_res2.config(
            text=f"Porcentaje de rechazos: {p_rechazos:.2f}% | Pasos de Integración (Euler): {len(datos_e)}")

        # Poblar Pestaña 1
        self.poblar_tabla(self.tree_principal, col_p, datos_p)
        # Poblar Pestaña 2
        self.poblar_tabla(self.tree_euler, col_e, datos_e)

    # [NUEVO EULER]: Modularizamos el rellenado visual para soportar ambas tablas
    def poblar_tabla(self, tree, columnas, datos):
        tree.delete(*tree.get_children())
        tree["columns"] = columnas

        for col in columnas:
            tree.heading(col, text=col)
            # Estética manual para ajustar los anchos y que no se corte el texto
            if "_ID" in col:
                w = 40
            elif "Emp" in col:
                w = 110
                ...
                w = 75
            elif "Instancia" in col:
                w = 120
            elif "t (" in col or "Derivada" in col or "Prox" in col:
                w = 110
            else:
                w = 80

            tree.column(col, width=w, anchor=tk.CENTER, stretch=False)

        for fila in datos: tree.insert("", tk.END, values=fila)

    # [NUEVO EULER]: Función que le dice a Excel de qué pestaña sacar los datos según la que tengas abierta
    def obtener_tree_activo(self):
        id_pestaña_activa = self.notebook.index(self.notebook.select())
        return self.tree_principal if id_pestaña_activa == 0 else self.tree_euler

    def seleccionar_todo(self, event):
        # [COMANDO]: .selection_set() resalta en azul las filas para copiarlas.
        tree = self.obtener_tree_activo()
    ...
    def copiar_a_excel(self, event):
        ...
        if not seleccion: seleccion = tree.get_children()

        # Arma la primera fila uniendo los nombres de columna (headers) separados por \t.
        texto = "\t".join([tree.heading(c)["text"] for c in tree["columns"]]) + "\n"

        # Concatena cada fila seleccionada formateando sus celdas con \t.
        for item in seleccion: texto += "\t".join(str(v) for v in tree.item(item, "values")) + "\n"
