...
def simular_sistema(params):
    ...
    estado = {
        'num_evento': 0, 'llegadas': 0, 'rechazos': 0, 'salidas': 0,  # [CONDICIONES INICIALES]: Arrancamos en evento 0
    ...
    def_rnds = {
        ...
        'k_aplicado': "", 't_integracion': "", 't_lect': ""
    }
    ...
    def commitear_buffer_a_grilla(forzar=False):
        ...
        if forzar or (buffer_reloj >= params['desde_reloj'] and len(datos_grilla) < params['filas_mostrar']):
            ...
            fila = (
                       ...
                       r_rnd_pag, r_paginas, rnds['k_aplicado'], r_t_int, r_t_lect,
    ...
    # ====================================================================
    # [CONDICIONES INICIALES]: Inyección de Fila 0 en el Buffer
    # ====================================================================
    def inicializacion():
        registrar_fila("Condiciones Iniciales")
        yield env.timeout(0)
    ...
    def generador_llegadas():
        ...
        # [OFFSET DE LLEGADA]: Pausamos la fábrica antes de hacer el primer cliente.
    ...
    # Registramos la inicialización estática y luego la fábrica
    env.process(inicializacion())  # [CONDICIONES INICIALES]
    ...
    columnas_base = [
        ...
        "RND Pág", "Páginas", "Valor K", "T. Integración (t)", "T. Lectura (min)",
...
class VentanaSimulacion:
    ...
    def obtener_parametros(self):
        ...
        if tiempo_simulacion <= 0: errores.append("X (tiempo a simular) debe ser > 0.")
        if limite_iteraciones <= 0: errores.append("N (máx. llegadas) debe ser > 0.")
        if filas_mostrar <= 0: errores.append("i (cant. filas a mostrar) debe ser > 0.")
        if desde_reloj < 0: errores.append("j (mostrar desde min.) debe ser >= 0.")
        if desde_reloj >= tiempo_simulacion: errores.append("j (mostrar desde min.) debe ser menor que X.")
        if h <= 0: errores.append("h (paso Euler) debe ser > 0.")
        if capacidad <= 0: errores.append("Capacidad máxima debe ser > 0.")
        if t_llegada <= 0: errores.append("Llegadas (Cte) debe ser > 0.")
        if abs(p_pedir + p_dev + p_cons - 100.0) > 0.01: errores.append("Probabilidades deben sumar 100%.")
        if not (0 <= p_queda <= 100): errores.append("% Se Queda a Leer debe estar entre 0% y 100%.")
        if m_pedir <= 0: errores.append("Media Pedir Exp(-) debe ser > 0.")
        if u_dev_a >= u_dev_b: errores.append("Unif. Dev: A debe ser menor que B.")
        if u_cons_a >= u_cons_b: errores.append("Unif. Cons: A debe ser menor que B.")
        if u_pag_a >= u_pag_b: errores.append("Unif. Pág: A debe ser menor que B.")
        if k1 <= 0: errores.append("K (Pág < 200) debe ser > 0.")
        if k2 <= 0: errores.append("K (Pág < 300) debe ser > 0.")
        if k3 <= 0: errores.append("K (Pág >= 300) debe ser > 0.")
        ...
        return {
            'tiempo_simulacion': tiempo_simulacion, 'limite_iteraciones': limite_iteraciones,
            'desde_reloj': desde_reloj, 'filas_mostrar': filas_mostrar, 'h': h, 'capacidad': capacidad,
            't_llegada': t_llegada, 'p_pedir': p_pedir / 100.0, 'p_dev': p_dev / 100.0,
            'p_queda': p_queda / 100.0, 'm_pedir': m_pedir, 'u_dev_a': u_dev_a, 'u_dev_b': u_dev_b,
            'u_cons_a': u_cons_a, 'u_cons_b': u_cons_b, 'u_pag_a': u_pag_a, 'u_pag_b': u_pag_b,
            'k1': k1, 'k2': k2, 'k3': k3
        }
